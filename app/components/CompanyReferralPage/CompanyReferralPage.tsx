...
type T_CompanyReferralPage = {
  platformSetting: T_PlatformSetting;
  referral: null | T_Referral;
};

export const CompanyReferralPage = ({
  platformSetting,
  referral,
}: T_CompanyReferralPage) => {
  ...
  const { t } = useTranslation(namespaces.companyReferral);
  const { t: tCommon } = useTranslation(namespaces.common);
  const { user } = useUser();
  const submit = useSubmitWithActions();
  const { getLocalizedRoute } = useLocalizedRoute();

  const accessToCreateReferral =
    user?.role === E_Roles.B2B_OWNER && !referral?.code;
  ...
  const handleCreateReferral = useCallback(() => {
    ...
    submit(null, {
      action: getLocalizedRoute({
        route: E_Routes.companyReferral,
      }),
      method: "post",
    });
  }, [user]);
  ...
  return (
    <Section
      breadcrumbs={[E_Routes.home, E_Routes.company, E_Routes.companyReferral]}
      buttons={
        <>
