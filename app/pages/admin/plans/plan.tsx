...
const AdminPlanEditPage = dynamic(() =>
  import("~/components/AdminPlanEditPage").then(module => ({
    default: module.AdminPlanEditPage,
  })),
);

export const handle = getI18nextNamespaces({
  extraNamespaces: [namespaces.adminPlanEdit],
});
      ...
      >
        {data => (
          <RespectUser userRoles={[E_Roles.ADMIN_SUPER]}>
            <AdminPlanEditPage plan={data.plan} />
          </RespectUser>
        )}
      </RespectSchema>
    </RespectLocalization>
  ...
  try {
    ...
    switch (request.method) {
      case E_Requests.patch: {
        return await updatePlanAdmin({ request, userId, userSessionVersion });
      }
      case E_Requests.delete: {
        return await deletePlanAdmin({ request, userId, userSessionVersion });
      }
    }
