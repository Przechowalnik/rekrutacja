...
import {
  ...
  postAutocomplete,
} from "~/data/autocompleteAddress.server";
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    if (request.method === E_Requests.post) {
      return await postAutocomplete({ request });
    } else if (request.method === E_Requests.put) {
      return await getPlaceAutocomplete({ request });
    }
